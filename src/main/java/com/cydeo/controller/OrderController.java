...
public class OrderController {
    ...
    public String orderForm(@RequestParam UUID pizzaId, Model model) {
        ...
        pizzaOrder.setPizza(getPizza(pizzaId)); //set the pizza for order
    ...
    @PostMapping("/{pizzaId}")
    public String processOrder(@PathVariable UUID pizzaId, PizzaOrder pizzaOrder) { // this @PathVariable("pizzaId") is optional

        // Save the order

        pizzaOrder.setPizza(getPizza(pizzaId)); // If there is no field in the form, pizzaOrder.setPizza(null) will run
